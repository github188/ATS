...
void ats_devpb_probe()
{
    ...
    TiXmlElement    *dev_node = mroot->FirstChildElement("device");
    osa_assert(dev_node != NULL);
    
    TiXmlElement    *info_node = dev_node->FirstChildElement("info");
...
osa_bool_t ats_devpb_is_support(ats_devpb_t *dp, const osa_char_t *dev_name)
{
    ...
    memcpy(devs, dp->dev_support, strlen(dp->dev_support));
