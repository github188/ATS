...
SDK_MediaFD MainWindow::playLiveMedia(Device *dev)
{
    if (!dev)
    {
        return -1;
    ...
    GUI_LogInfo("Play live media for device : %s\n", dev->getDevName().c_str());
    SDK_Layer *devSdk = sdkMng.devMatchSdk(dev);
    if (!devSdk)
    {
        GUI_LogError("Device : %s, no suitable SDK for it\n", dev->getDevName().c_str());
        return -1;
    }

    DeviceInfo devInfo = dev->getDevInfo();
    osa_err_t err;

    err = devSdk->SDK_Init();
    if (err != OSA_ERR_OK)
    {
        GUI_LogError("Failed to initialize SDK named %s\n", devSdk->nameSpace);
        return -1;
    }
    ...
    {
        GUI_LogInfo("Login device (%s) succeed!\n", dev->getDevName().c_str());
    }

    HWND playWnd = ui->graphicsView->winId();

    mfd = devSdk->SDK_PlayLiveMedia(1, 0, &playWnd);
    if (mfd <= 0)
    {
        GUI_LogError("Failed to play live media!\n");
        return -1;
    }
    else
    {
        GUI_LogInfo("start playing live media......\n");
        return mfd;
    }
}

void MainWindow::stopLiveMedia(Device *dev)
{
    if (!dev)
    {
        return;
...
void MainWindow::onTreeViewRightClicked(QPoint pos)
{
    ...
    QAction *actionPlayMedia = new QAction(tr("PlayMedia"), this);
    ...
    actionPlayMedia->setData(QVariant("PlayMedia"));
    ...
    menu->addAction(actionPlayMedia);
...
void MainWindow::onTreeViewMenuShowed(QAction *action)
{
    ...
    else if (act == QVariant("PlayMedia"))
    {
        if (currDev->playMedia() != OSA_ERR_OK)
        {
            return;
        }

        playLiveMedia(currDev);
    }
