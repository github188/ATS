...
void    ATS_SysInit()
{
    ...
    if (!sysServer)
    {
        ATS_LogError("Failed to open agent socket!\n");
        return;
    }
    ...
    if (err != OSA_ERR_OK)
    {
        ATS_LogError("Failed to listen socket!\n");
    }
...
void    ATS_SysExec()
{
    ATS_LogInfo("Start system message loop!\n");
    ...
    while (1)
    {
        ...
        else if (val == OSA_SOCK_HAS_CONNECT)
        {
            for (idx=0; idx<CLIENT_MAX; idx++)
            {
                if (!clientSock[idx])
                {
                    ...
                    ATS_LogInfo("New client connected : %d\n", clientSock[idx]->fd);
                    break;
                }
            }
        }
        ...
        {
            ATS_LogInfo("Received data from client!\n");
            ...
            for (idx=0; idx<CLIENT_MAX; idx++)
            {
                if (clientSock[idx] && osa_tcpserver_client_ready(sysServer, clientSock[idx]) == OSA_TRUE)
                {
                    ...
                    {
                        ATS_LogError("buffer is too small !\n");
                        buf[sizeof(buf)-1] = '\0';
                    }
...
static osa_size_t  _sysIOCHWrite(osa_ioch_t *self, osa_char_t *data, osa_size_t size)
{
    ...
    {
        ATS_LogError("Please open I/O channel first!\n");
        return 0;
    }
